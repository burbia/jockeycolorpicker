...
(function($) {
  ...
  // Public core
  $.paletteElement = function( el, options ) {
    var
      ns = 'palette-element', // Base attr / class
      $el = $(el),
      plugin = this,
    ...
    if (options.colors.length>0){
        //var test = jQuery.makeArray(options.colors[0]);
        if ("string" === typeof options.colors[0]){
            current_value = options.colors[0]
        }else{
            $.each(options.colors[0], function(key, element) {
    ...
    plugin.init = function() {
        ...
        var
          ...
          $sw = $('<span>').addClass('swatch')
            .attr({
              ...
              'data-value': col,
              'data-name': key
            }).css('background', 'url('+plugin.settings.imgPath+col+'.png) center center no-repeat').css('background-size','70px 70px').css('height','70px');


        // console.log(key + '===' +  current_value + ' - > '+(key === current_value)+' - t -'+(typeof key)+' '+(typeof current_value));
        if ( key === current_value || col === current_value ) {
          $sw.addClass('active');
          $button.css('background', 'url('+plugin.settings.imgPath+col+'.png) center center no-repeat').css('background-size','26px 24px');;
        }
      ...
      } else if (plugin.settings.clear_btn === 'first') {
      ...
      .on('click', 'span.swatch', function(e){
        ...
        if ( $(e.target).is('.clear') ) {
          ...
          $button.css('background',$(e.target).css('background'));
          $button.css('background-size','26px 24px');
        }
  ...
  $.fn.paletteElement = function(options) {
    return this.each(function() {
      if (typeof $(this).data('paletteElementPlugin') === 'undefined') {
        $(this).data('paletteElementPlugin', new $.paletteElement(this, options));
      }
    });
  };
